# run-ui Specification

## Purpose

Expose the loopback run API and WebSocket event stream, manage the runtime server lifecycle from the Electron thin shell, and render runs on a read-only React Flow canvas without authoring or later-phase controls.
