...
| **放行** | 实现 PR：三个 required check 全绿（ci / spec-validate / test-guard）+ 看顾问评审（已自动发）→ 你 merge（issue 自动关闭）。顾问运行失败或复审仍为 `CHANGES` 都会停止自动循环交你处理；顾问不是 required gate。**人是终审** |
...
- **修复**：watch 收到首次 `VERDICT: CHANGES` 后，自动把最新顾问评论回喂原 builder，在同一 worktree 最小复修并复审一次。复修失败、顾问运行失败或二次 `CHANGES` 都停止自动循环交人；顾问始终不是 required gate。手动接管同一路径时用 `node scripts/dispatch.mjs <issue#> <同一 builder> <PR#>` 回喂评论，也可本地进该 worktree 续修。
