...
| **审提案** | watch 起草的提案 PR：**逐条审验收判据——这是奖励函数，全流程最值得花时间的一步** → merge。merge 后 watch 自动播种实现 issues（自带 ready + 判据，挂为源需求 issue 的 sub-issues——父 issue 上看树形进度）并逐个派发 → 开 PR → 自动顾问评审（最多一轮复修/复审）；同 change 的 issue 串行防语义冲突 |
| **放行** | 实现 PR：四个 required check 全绿（ci / spec-validate / test-guard / advisor-review）+ 读顾问评审 → 你 merge（issue 自动关闭）。`advisor-review` 只卡「顾问评论已存在」——评审进行中挂 pending，评论发出置绿（新 push 需新一轮评论）；超时 / 失败自动放行并打 `needs-human`（D15），评审结论从不阻塞。顾问运行失败或复审仍为 `CHANGES` 都会停止自动循环交你处理，watch 会在 PR 留简短交接评论（手机上可见）。**人是终审** |

不挂 watch 时底层脚本可手动跑（调试 / 接管）：`propose.mjs <issue#>`、`seed-issues.mjs <change> [milestone|-] [phase|-] [parent-issue#]`、`dispatch.mjs <issue#>`、`review.mjs <PR#>`。
...
node scripts/review.mjs <PR#>         # 用 Claude 评审（读码 + 实机验证）→ 发顾问评论
...
- 顾问评审的**结论不是门禁**：抓不抓得到问题都不阻塞合并；真正的关卡是确定性检查 + 你的终审。required check `advisor-review` 只保证你 merge 时顾问评论已经在场（或已明确放弃并打 `needs-human`）——防手快，不判对错（D15）。评审子进程有硬超时（缺省 20 分钟，`REVIEW_TIMEOUT_MINUTES` 可调），超时整树击杀、门禁自动放行。
- reviewer 在 PR head 的临时 detached worktree（一次性副本，评审毕即删）里运行：读码之外，还应实机执行验证命令（pnpm install / test / smoke 等）核实判断，能跑就不许标"存疑"。写入落不到主工作树；上报（gh）只由脚本做——claude 靠 Bash 白名单、codex 靠 sandbox、opencode 靠隔离 + prompt 约束。diff 与判据落文件传路径——省 token。
- GUI 改动的验证分两条 lane，不重复：CI 的确定性 smoke/acceptance 断言已知不变量（回归面）；reviewer 经 Playwright MCP（CDP 接管 Electron 窗口）按 diff 探索实际交互（本次改动面）——prompt 要求它不重跑 CI 已证明的断言。GUI lane 目前仅 claude 适配器接了 MCP。
