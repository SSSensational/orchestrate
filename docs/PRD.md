...
**取舍**：AgentAdapter 接口设计成对上层透明（§6），A、B 均可挂进 registry。Phase 1 从一条最顺手的原生通道起步；能力由 capability 声明暴露给 Validator。
...
Natural language / Canvas / Template / JSON editor
   ...
   Orchestrator（依赖调度 + 节点状态机 + 记忆化重遍历 + 活性看门狗）
        ...
        ▼
   Web UI（画布 + inspector + 事件时间线 + artifact 面板 + agent 实时流）
```
...
后端     Hono（HTTP + WebSocket），单进程
存储     SQLite（better-sqlite3），第 11 节 6 张表
校验     zod（L1）+ 自写图语义校验器（L2/L3）
前端     Vite + React + @xyflow/react（React Flow）
Agent    AgentAdapter registry；Phase 1 起步一条原生通道，Phase 2 覆盖 Claude Code / Codex / OpenCode
...
**Phase 1 — 单通道跑通**：实现第一个 adapter；写死 Cross-Agent Review IR（无环、单 agent 版）；L1+L2 最小校验；状态机 + run_events 落库；final text 兜底成 artifact；UI 显示节点状态与实时流。

**Phase 2 — 多 agent 并行 + 工具通道**：补齐 Claude Code / Codex / OpenCode 三家 adapter（事件归一化单测）；workflow MCP server（emit_artifact/get_context + task-scoped token）；`agent.reduce`；失败重试（attempt/failure_reason/带毒判定）；UI artifact 溯源。

**Phase 3 — Human Gate + 恢复**：gate 四要素；受控环（max_rounds）；恢复指针持久化 + 杀进程重启续跑；时间线按 seq 回放（真 e2e：spawn → kill -9 → restart → assert）。

**Phase 4 — Visual Authoring**：React Flow 增删改；ui_json sidecar；Validator 错误画布高亮；画布只产出 IR。
